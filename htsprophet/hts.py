...
def forecast_hts(y, h=1, nodes=None, method='OLS', freq='D', transform=None, include_history=True, cap=None, capF=None,
    ...
    ##
    theDictionary, y, ynew = fit_predict(boxcoxT, cap, capF, changepoint_prior_scale, changepoints, daily_seasonality,
                                         freq, h, holidays, holidays_prior_scale, include_history, interval_width,
                                         mcmc_samples, method, n_changepoints, nodes, numThreads,
                                         seasonality_prior_scale, skipFitting, uncertainty_samples, weekly_seasonality,
                                         y, yearly_seasonality)
    ##
...
def fit_predict(boxcoxT, cap, capF, changepoint_prior_scale, changepoints, daily_seasonality, freq, h, holidays,
                holidays_prior_scale, include_history, interval_width, mcmc_samples, method, n_changepoints, nodes,
                numThreads, seasonality_prior_scale, skipFitting, uncertainty_samples, weekly_seasonality, y,
                yearly_seasonality):
    theDictionary = {}
    if method == 'cvSelect':
        ynew = fit_predict_CV(boxcoxT, cap, capF, changepoint_prior_scale, changepoints, daily_seasonality, freq, h,
...
def fit_predict_CV(boxcoxT, cap, capF, changepoint_prior_scale, changepoints, daily_seasonality, freq, h, holidays,
    ...
    for trainIndex, testIndex in tscv.split(y.iloc[:, 0]):
        ...
        for key in ynew1.keys():
            MASE1.append(
                np.mean(abs(ynew1[key].yhat[-len(testIndex):].values - y1.iloc[testIndex, key + 1].values)))
            MASE2.append(
                np.mean(abs(ynew2[key].yhat[-len(testIndex):].values - y1.iloc[testIndex, key + 1].values)))
            MASE3.append(
                np.mean(abs(ynew3[key].yhat[-len(testIndex):].values - y1.iloc[testIndex, key + 1].values)))
            MASE4.append(
                np.mean(abs(ynew4[key].yhat[-len(testIndex):].values - y1.iloc[testIndex, key + 1].values)))
            MASE5.append(
                np.mean(abs(ynew5[key].yhat[-len(testIndex):].values - y1.iloc[testIndex, key + 1].values)))
            MASE6.append(
                np.mean(abs(ynew6[key].yhat[-len(testIndex):].values - y1.iloc[testIndex, key + 1].values)))
            MASE7.append(
                np.mean(abs(ynew7[key].yhat[-len(testIndex):].values - y1.iloc[testIndex, key + 1].values)))
    ##
...
# def compute_nodes(sales_long: pd.DataFrame, level_columns: List[str]):

#     # level_columns = ['store_id', 'cat_id', 'dept_id']

#     levels = [(col, values) for col in level_columns for values in [sales[col].unique()]]
#     levels = OrderedDict(levels)

#     current_level, current_values = levels.popitem(last=False)
#     nodes = [[len(current_values)]]

#     while len(levels) > 0:
#         next_level, next_values = levels.popitem(last=False)
#         l = []
#         for value in current_values:
#             sales_ = sales[sales[current_level] == value]
#             l.append(sales[next_level].nunique())
#         nodes.append(l)
#         current_level, current_values = next_level, next_values
